...
def call_api_endpoint(
    ...
    try:
        with httpx.Client(base_url=base_url) as client:
            logger.debug(
                f"base_url: {base_url}, api_endpoint: {api_endpoint}"
            )
        ...
        logger.error(
            f"HTTP status error ocurred: {e.response.text}"
        )
        ...
        return httpx.Response(
            status_code=500, 
            content=b"Network error"
        )
    except Exception as e:
        logger.error(f"Unexpecter error ocurred: {str(e)}")
        return httpx.Response(
            status_code=500,
            content=b"Unexpected error"
        )